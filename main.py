...
def init_db():
        conn = get_conn()
        conn.executescript("""
            CREATE TABLE IF NOT EXISTS users (telegram_id INTEGER PRIMARY KEY, username TEXT, first_name TEXT, age INTEGER, gender TEXT, height REAL, weight REAL, target TEXT, calories_goal REAL, trial_start TEXT, is_vip INTEGER DEFAULT 0);
            CREATE TABLE IF NOT EXISTS food_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, telegram_id INTEGER, food_name TEXT, calories REAL, protein REAL, fat REAL, carbs REAL, logged_at TEXT DEFAULT CURRENT_TIMESTAMP);
            CREATE TABLE IF NOT EXISTS payment_requests (id INTEGER PRIMARY KEY AUTOINCREMENT, telegram_id INTEGER, photo_file_id TEXT, status TEXT DEFAULT 'pending', created_at TEXT DEFAULT CURRENT_TIMESTAMP);
        """)
        conn.commit()
        conn.close()
...
async def handle_food_photo(message: types.Message):
        try:
                    await message.answer("\U0001f50d Rasm tahlil qilinmoqda, iltimos kuting...")
            ...
            \U0001f37d Ovqat: [nom]
            \U0001f525 Kaloriya: [miqdor] kcal
            \U0001f4aa Oqsil: [miqdor] g
            \U0001f9c8 Yog': [miqdor] g
            \U0001f35e Uglevodlar: [miqdor] g"""

        # Yangi google-genai SDK (>=1.0) uchun to'g'ri sintaksis
        ...
        await message.answer("\u274c Tahlil qilishda xatolik yuz berdi. Qaytadan urinib ko'ring.")
